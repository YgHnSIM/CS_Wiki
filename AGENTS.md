...
- 의미 관계는 본문의 `## 관계` 표에 방향·대상·설명·근거를 함께 기록한다. 대상은 위키링크여야 하고, 근거 칸은 소스·참고 자료 페이지만 받는다.
- 그래프에 공개하는 개념·개체·분석은 비어 있지 않은 `capability_layers`와 `history.layer`를 가진다. 연도는 원전 대조 없이 추정하지 않는다.
