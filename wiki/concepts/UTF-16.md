...
[[UTF-16]]은 Unicode scalar value를 16비트 [[코드 유닛]] 하나 또는 두 개로 표현하는 인코딩 형식이다. `U+0000`부터 `U+D7FF`, `U+E000`부터 `U+FFFF`까지는 하나의 16비트 코드 유닛으로 표현되고, `U+10000`부터 `U+10FFFF`까지는 surrogate pair 두 코드 유닛으로 표현된다.

## Surrogate pair

Surrogate pair는 높은 surrogate와 낮은 surrogate가 쌍을 이루어 하나의 scalar value를 나타내는 방식이다. surrogate 코드 포인트 자체는 Unicode scalar value가 아니므로, 단독 surrogate 코드 유닛은 ill-formed UTF-16이다.
