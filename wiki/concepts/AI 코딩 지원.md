...
[[AI 코딩 지원]]은 개발자가 작성 중인 코드, 자연어 요청, 편집기나 저장소의 맥락을 바탕으로 구현 후보·설명·수정안·테스트 등을 제안받고, 사람이 이를 선택·수정·검증하는 작업 방식이다. 완전 자율 실행 여부와 무관하게, 도구의 출력은 현재 시스템의 요구와 결과 계약 안에서 평가되어야 한다.

AI 코딩 지원의 효과는 제안 생성 속도뿐 아니라 요청의 경계, 저장소 맥락, 모델·도구 판본, 개발자의 숙련도, 테스트·리뷰·배포 기준에 따라 달라진다. 따라서 특정 과제에서의 시간 단축이나 지연을 도구 자체의 고정 성질로 바꾸지 않는다.
