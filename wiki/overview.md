...
운영 상태: 전체 226개 페이지 중 active 192개, draft 34개, review 0개, archived 0개다.
...
[[VisiCalc Apple II Version 1.0 Manual]]은 셀에 값과 수식을 함께 두고 입력 변경을 관련 결과에 자동으로 전파한 1979년의 작동을 보여 준다. [[Excel formula recalculation and dependencies]]는 이 구조를 선행·종속 셀, 상대·절대 참조, 자동·수동 재계산, 순환 참조와 수치 정밀도의 문제로 확장한다. [[수학 표에서 스프레드시트로]]는 이를 인쇄 표의 단순한 디지털화가 아니라 완성된 답의 보관소가 최종 사용자의 실행·시나리오 모형으로 바뀐 전환으로 분석하며, 수식·참조·계산 상태와 변경 이력이 새로운 검산 대상이 됐음을 강조한다.
...
[[수학 표에서 스프레드시트로]]는 미리 계산된 값을 배포한 수학 표와 셀 수식·의존 관계를 재계산하는 스프레드시트를 비교해, 표의 사용자가 조회자에서 계산 모형의 작성자·검토자로 바뀐 과정을 분석한다.
