...
처음 읽는 사용자는 10개의 학습 경로에서 컴퓨팅의 기원, 계산 가능성, 저장 프로그램 컴퓨터, 초기 소프트웨어, 구조적 프로그래밍, Fortran과 컴파일러, Unix와 C, 재사용과 API, 논리 회로, 유니코드의 흐름을 순서대로 따라갈 수 있다. 개별 문서의 근거 추적 영역은 직접 근거와 보조 근거, 원본 보존 상태, 원문 URL을 한 흐름으로 연결한다.
