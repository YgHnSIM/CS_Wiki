...
역사·인과 렌즈는 같은 222개 공개 문서를 연도 목록이 아니라 **한계 → 대응 → 새 능력 → 다음 제약**의 변화로 읽게 한다. 현재 원전 대조로 연도가 기록된 문서는 139개이고, 그중 사건 시점을 가진 문서 38개와 출판 시점을 가진 문서 101개를 1800년 이전부터 2015년 이후까지 여덟 시대에 배치한다. 연도를 확인하지 못한 148개는 제목이나 작성일로 추정하지 않고 별도 `연도 미상` 시대에 보존한다. 편집자가 직접 기록한 `responds_to`, `enables`, `precedes`, `constrains`만 합성한 검토 전환은 17개이며 완전 전환 9개와 부분 전환 8개를 구분해 보여 준다.
...
근거 계보 렌즈는 192개 공개 지식 문서, 95개 정규 소스·참고 자료와 816개 문서–근거 연결을 **원자료·재현 정보 → 정규 소스·참고 자료 → 지식 문서·검토 관계**의 3레일로 펼친다. 프론트매터 `sources`는 문서 전체의 등록 근거 묶음으로, 설명과 직접 근거가 붙은 편집 관계 44개는 별도의 검토 관계로 구분한다. 근거 수를 신뢰도·진실도 점수로 만들지 않고, 같은 자료를 쓰는 문서도 공통 자료를 등록했다는 사실만 보여 준다. 근거 문서의 로컬 원본 6개, 보존 스냅샷 7개, 외부 링크 의존 82개는 품질 순위가 아니라 재현 조건으로 표시한다.
