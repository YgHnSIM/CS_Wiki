...
이 위키는 컴퓨터 과학과 컴퓨팅사 지식을 원본 소스 기반으로 정리한다. 현재 정규 소스는 6개 묶음, 참고 자료는 4개이며, 주요 주제는 19세기 기계식 계산 장치에서 20세기 초기 저장 프로그램 컴퓨터의 계보, 로더, 어셈블러, 서브루틴, 스택 기반 호출 구조, 서브루틴 라이브러리와 API적 사고, 논리 회로와 이진 표현, 그리고 디지털 데이터 표현과 메모리 안전성으로 이어지는 컴퓨팅 개념의 형성이다.
