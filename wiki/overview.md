...
이 위키는 컴퓨터 과학과 컴퓨팅사 지식을 원본 소스 기반으로 정리한다. 현재 정규 소스는 6개 묶음, 참고 자료는 110개이며, 주요 주제는 수학 표와 인간 계산 조직, 검산과 수치 결과 계약, 정적 표에서 재계산 가능한 스프레드시트로의 전환, 19세기 기계식 계산 장치에서 계산 가능성, 계산 복잡도와 다항 시간 환원, 범용성과 프로그래밍 가능성, 개발 생산성의 다차원 측정, 기계 계산의 이론적 의미, 20세기 초기 저장 프로그램 컴퓨터의 계보, 로더, 어셈블러, 서브루틴, 스택 기반 호출 구조, `goto`와 구조적 프로그래밍, Fortran과 컴파일러, Unix와 C, 서브루틴 라이브러리와 API적 사고, 소프트웨어 재사용의 역사, 집적도·소자 축소에서 캐시·RISC와 메모리 장벽, 알고리즘·소프트웨어·아키텍처의 공동 성능 개선, 부동소수점 표준화와 수치 정확성, 작업·결과·경계·규칙을 함께 고정하는 벤치마크 계약, 혼합 정밀도와 목표 품질 도달 시간, Transformer의 자기 주의·입출력 인지 실행·연속 배칭·KV 캐시·접두사 재사용·추측 디코딩·프리필/디코드 분리, TTFT·TPOT goodput과 유효 요청당 에너지·가속기 시간 측정, 고정 크기와 확대 크기의 병렬 확장, 장기 환산 계산량/에너지·부하별 전력·활성 면적으로 나뉜 전력 장벽, 팬아웃과 꼬리 지연, CAP·복제 로그 합의·외부 일관성의 상태 계약, 결함 허용과 탐지·복구를 포함한 서비스 결과 계약, 디코드·정규화·입력 검증·수치 범위·메모리 경계를 포함한 데이터 해석의 보안 계약, 분산·도메인 특화 구조로 이어진 컴퓨팅 능력의 발달, 논리 회로와 이진 표현, 디지털 데이터 표현과 메모리 안전성, 그리고 유니코드 기반 텍스트 인코딩 심화로 이어지는 컴퓨팅 개념의 형성이다.


<!-- wiki-maintenance: status-summary -->
운영 상태: 전체 340개 페이지 중 active 340개, draft 0개, review 0개, archived 0개다.
...
이어 [[CAP 정리]], [[복제 로그와 합의]], [[외부 일관성과 시간 불확실성]]을 추가해 분할 중의 응답 약속, crash fault 합의의 상태 순서, 전역 트랜잭션의 시간 불확실성·commit wait를 분리했다. [[분산 서비스는 빠른 응답과 같은 상태를 어떻게 함께 보장하는가]]는 이를 사용자 기한·복구·자원 비용을 포함한 서비스 결과 계약으로 연결한다.

최근에는 [[입력 검증]]과 [[입력을 해석하는 경계는 왜 보안 경계인가]]를 추가해 디코드·정규화·검증·인가·출력 처리의 역할을 구분했다. 이 연결은 [[동형이의 문자]]의 식별자 정책과 [[정수 오버플로]]의 길이·오프셋·할당 크기 계산을 같은 해석 경계의 문제로 읽게 한다.

[[표현·배치·계약은 어떻게 컴퓨팅 능력을 만드는가]]는 이 주제들을 실행 가능한 표현, 작업·상태의 배치, 유효 결과 계약이라는 세 횡단 경계로 다시 읽는다. 작업표와 스케줄러, 결과·절차·KV 상태의 재사용, 수치·텍스트·분산 상태의 동등성을 직접 계보가 아닌 비교 질문으로 연결한다.

이번 확장에서는 [[동시성]], [[상호 배제와 동기화]], [[순차 일관성]], [[선형화 가능성]]을 추가해 프로그램 내부의 제어 흐름이 여러 실행 주체의 순서·공유 상태·원자성 계약으로 바뀌는 과정을 정리했다. [[한 프로그램의 순서는 여러 실행 주체에서 어떻게 보존되는가]]는 이 경계를 복제 상태와 사용자 관찰 순서에 연결한다.

[[중간 표현]], [[이기종 실행 모델]], [[알고리즘과 스케줄 분리]], [[성능 이식성]]은 한 소스 프로그램의 의미를 서로 다른 CPU·GPU·가속기와 메모리 계층에 매핑할 때 의미 보존과 성능 목표를 분리하는 틀을 제공한다. [[한 소스의 의미와 성능은 가속기로 어떻게 번역되는가]]는 LLVM, CUDA, Halide, TVM을 이 틀에서 비교한다.

[[종단 간 원칙]], [[원격 프로시저 호출]], [[부분 실패]], [[분산 파일 시스템]]은 로컬 호출·파일 인터페이스가 원격 상태로 확장될 때 지연, 재시도, 멱등성, 복제와 순서의 계약을 드러낸다. [[자동화 경계]], [[검증 노동]], [[AI 코딩 지원]]은 도구의 생성량이 아니라 사람에게 남는 이해·테스트·통합 책임을, [[수명 주기 평가 경계]], [[운영 탄소와 내재 탄소]], [[전력 사용 효율]]은 운영 에너지와 전체 환경 영향의 측정 범위를 각각 정리한다.

## 웹 탐색 구조

웹사이트는 정규 소스와 참고 자료를 분리하고, 개념·인물·분석 문서를 자료 유형별로 탐색할 수 있게 구성한다. 검색은 제목뿐 아니라 별칭, 태그, 본문, 출처 ID를 함께 대상으로 삼고, 자료 유형과 검증 상태 필터를 제공한다. 빌드는 [[지식 그래프 관계 스키마]]에 따라 본문 언급, 읽기 추천, 근거, 학습 순서와 편집 관계를 하나의 정규화 데이터로 만든다. 같은 문서쌍의 여러 간선은 한 연결 묶음으로 접고 **핵심·읽기·근거·언급** 네 채널로 나눈다. 각 지식 문서는 한 번에 최대 6개 연결만 텍스트 목록으로 먼저 보여 주고, 공간 그래프는 필요할 때 펼친다. 두 문서 연결 탐색기는 임의의 출발·도착 문서 사이에서 최대 세 경로를 찾아 각 중간 문서, 관계 이유와 기록 방향을 설명한다. 학습 노선 지도는 25개 경로를 순서가 있는 역으로 보여 주며, 역을 선택하면 그 문서를 공유하는 모든 노선이 동시에 켜지고 같은 역을 유지한 채 다른 질문으로 갈아탈 수 있다.

역사·인과 렌즈는 336개 공개 문서를 연도 목록이 아니라 **한계 → 대응 → 새 능력 → 다음 제약**의 변화로 읽게 한다. 현재 원전 대조로 연도가 기록된 문서는 170개이고, 그중 사건 시점을 가진 문서 39개와 출판 시점을 가진 문서 131개를 1800년 이전부터 2015년 이후까지 여덟 시대에 배치한다. 연도를 확인하지 못한 166개는 제목이나 작성일로 추정하지 않고 별도 `연도 미상` 시대에 보존한다. 편집자가 직접 기록한 `responds_to`, `enables`, `precedes`, `constrains`만 합성한 검토 전환은 158개이며 완전 전환 78개와 부분 전환 80개를 구분해 보여 준다.

아홉 시대는 10개 정적 조각으로 제공된다. 한 조각은 최대 120개 문서와 96개 전환 요약만 담고, 첫 화면은 manifest와 시대별 대표 문서만 받는다. 직접 선택은 안정 ID를 해시한 최대 96레코드 조회 버킷 하나와 해당 시대 조각만 불러오며, 전체 검색도 여덟 개의 작은 버킷을 병렬로 읽는다. 전환 하나가 관계를 매우 많이 가져도 시대 조각에는 최대 6개 대표 간선·32KiB 요약만 두고, 전체 관계는 최대 48레코드·64KiB 상세 쪽으로 나눠 사용자가 한 쪽씩 지연 로드한다. 새 공개 문서는 자동으로 정확히 한 시대에 들어가며 역사 메타데이터가 없으면 `연도 미상`·`층위 미분류`에 안전하게 수용된다. 시대 문서나 고차수 관계가 늘면 새 정적 조각만 생기므로 한 번의 요청과 화면 표시량은 전체 문서 수와 함께 커지지 않는다.

근거 계보 렌즈는 220개 공개 지식 문서, 116개 정규 소스·참고 자료와 949개 문서–근거 연결을 **원자료·재현 정보 → 정규 소스·참고 자료 → 지식 문서·검토 관계**의 3레일로 펼친다. 프론트매터 `sources`는 문서 전체의 등록 근거 묶음으로, 설명과 직접 근거가 붙은 편집 관계 438개는 별도의 검토 관계로 구분한다. 근거 수를 신뢰도·진실도 점수로 만들지 않고, 같은 자료를 쓰는 문서도 공통 자료를 등록했다는 사실만 보여 준다. 근거 문서의 로컬 원본 6개, 보존 스냅샷 7개, 외부 링크 의존 103개는 품질 순위가 아니라 재현 조건으로 표시한다.
...
처음 읽는 사용자는 25개의 학습 경로에서 컴퓨팅의 기원, 계산 가능성, 알고리즘 복잡도와 실용성, 저장 프로그램 컴퓨터, 초기 소프트웨어, 구조적 프로그래밍, Fortran과 컴파일러, Unix와 C, 재사용과 API, 논리 회로, 유니코드, 컴퓨팅 능력 발달, 범용 기계와 프로그래밍 가능성, 더 빠른 프로세서의 조건, 부동소수점 성능과 정확성, 병렬 확장성의 두 질문, 전력 장벽과 에너지 효율, 서비스 성능과 꼬리 지연, 결함 허용과 가용성, 동시성과 일관성, 소스 코드에서 이기종 가속기까지, 로컬 인터페이스에서 원격 상태까지를 순서대로 따라갈 수 있다. [[컴퓨팅 능력 독서 지도]]는 이 경로들을 빠른 진입, 핵심 분석 시리즈와 일곱 층위별 원전 심화로 다시 조합해 중복 없이 읽게 한다. 개별 문서의 근거 추적 영역은 문서 단위 근거와 자료 페이지가 기록한 원자료 및 보조·접근 자료, 원본 보존 상태와 원문 URL을 한 흐름으로 연결한다.

## 주요 주제

### 컴퓨팅의 기원

[[컴퓨팅의 기원 - 배비지와 러브레이스]]는 19세기의 계산 자동화, 범용 프로그램 가능성과 기호 조작의 출발점을 다룬다. [[The History of Mathematical Tables: From Sumer to Spreadsheets]]는 그보다 앞선 계산 결과의 저장·배포를, [[인간 계산자]]는 반복 산술을 작업표·[[검산]]·편집으로 조직한 노동 체계를 보강한다. 이 주제의 핵심 페이지는 [[수학 표]], [[인간 계산자와 계산 공장]], [[수학 표의 검산에서 수치 결과 계약으로]], [[스프레드시트]], [[수학 표에서 스프레드시트로]], [[찰스 배비지]], [[에이다 러브레이스]], [[차분 기관]], [[해석 기관]], [[유한 차분법]], [[알고리즘적 사고]], [[기호 조작]]이다.
...
[[VisiCalc Apple II Version 1.0 Manual]]은 셀에 값과 수식을 함께 두고 시트의 모든 수식을 행·열의 고정 순서로 자동 또는 수동 재계산한 1979년의 작동을 보여 준다. [[Excel formula recalculation and dependencies]]는 변경된 셀에 의존하는 수식을 다시 계산하는 구조와 선행·종속 셀, 상대·절대 참조, 순환 참조, 수치 정밀도와 변경 추적을 설명한다. [[수학 표에서 스프레드시트로]]는 이를 인쇄 표의 단순한 디지털화가 아니라 완성된 답의 보관소가 최종 사용자의 실행·시나리오 모형으로 바뀐 전환으로 분석하고, 고정 순회에서 의존성 기반 재계산으로의 변화와 새 검산 대상을 드러낸다.
...
[[GHG Protocol Product Life Cycle Accounting and Reporting Standard]]과 [[Chasing Carbon: The Elusive Environmental Footprint of Computing]]은 이 운영 지표를 장비 제조, 시설 인프라, 수명, 기능 단위와 배분까지 확장할 때 필요한 경계를 제시한다. [[낮은 운영 에너지는 지속 가능한 컴퓨팅을 보장하는가]]는 높은 처리량/W나 낮은 요청당 에너지가 수명 주기 환경 영향의 충분조건은 아니라는 점을 정리한다.
...
[[벤치마크 계약]]은 성능 숫자를 작업 부하, 결과 조건, 실행 시나리오, 시스템·계측 경계, 단위·분모, 규칙·판본의 묶음으로 읽는다. [[SPEC CPU 2026 Overview]], [[The Linpack Benchmark]], [[Roofline An Insightful Visual Performance Model]]은 실행 시간·처리량, 달성 성능·이론 상한, 병목 분석 모델이 서로 다른 질문임을 구분한다. [[MLPerf Training Benchmark]], [[MLPerf Inference Benchmark]], [[MLPerf Inference Power Measurement]], [[TokenPowerBench - Benchmarking the Power Consumption of LLM Inference]]는 품질·지연 조건과 전력·에너지 계측 경계까지 함께 고정한다. [[벤치마크 숫자는 왜 직접 비교되지 않는가]]는 같은 단위의 값도 이 계약이 다르면 단일 순위로 합칠 수 없는 이유를 분석한다.

### 혼합 정밀도와 AI 품질

[[Mixed Precision Training]]은 반정밀도 저장·연산에 단정밀도 마스터 가중치와 손실 스케일링을 결합해 심층 학습의 메모리·연산 비용과 수치 안정성을 함께 다룬다. [[MLPerf Training Benchmark]]은 처리량 향상이 목표 품질까지의 시간을 단축하지 않을 수 있음을 지적한다. [[혼합 정밀도]], [[목표 품질 도달 시간]], [[낮은 정밀도는 AI의 컴퓨팅 능력을 어떻게 바꾸는가]]는 낮은 비트 수 자체가 아니라 모델 품질·측정 규칙·자원 비용을 함께 기록하는 성능 비교를 제시한다.

추론에서는 [[GPTQ - Accurate Post-Training Quantization for Generative Pre-trained Transformers]]와 [[AWQ - Activation-aware Weight Quantization for On-Device LLM Compression and Acceleration]]가 [[LLM 가중치 양자화]]의 대표 PTQ 경로를 제공한다. 가중치 전용 저비트 표현은 모델 용량과 HBM 이동을 줄이지만 활성값·KV 상태를 자동으로 줄이지 않으며, 패킹·역양자화와 전용 커널이 필요하다. [[낮은 비트는 왜 LLM 추론 속도를 보장하지 않는가]]는 압축률과 과업 품질·종단 지연 사이의 조건을 분석한다.

### Transformer 실행과 LLM 서빙

[[Attention Is All You Need]]은 순환을 제거하고 [[자기 주의]]와 행렬 연산을 중심으로 한 [[Transformer]]를 제시해 학습 시 위치 병렬성을 높였다. [[FlashAttention - Fast and Memory-Efficient Exact Attention with IO-Awareness]]은 정확한 밀집 어텐션의 이차 산술을 없애지 않으면서 타일링·재계산으로 HBM 이동과 부가 메모리를 줄였다. [[입출력 인지 어텐션]]과 [[Transformer 추론은 왜 연산량만으로 설명되지 않는가]]는 학습 병렬성, 자동회귀 생성 순차성, FLOP와 데이터 이동을 서로 다른 성능 조건으로 정리한다.

[[Orca - A Distributed Serving System for Transformer-Based Generative Models]]는 모델 반복마다 요청을 다시 구성하는 [[연속 배칭]]으로 자동회귀 요청 사이의 병렬성을 활용했다. [[Efficient Memory Management for Large Language Model Serving with PagedAttention]]은 동적으로 커지는 [[KV 캐시]]를 블록으로 배치해 단편화와 중복을 줄였다. [[KV 캐시는 왜 LLM 추론 처리량을 제한하는가]]는 계산 슬롯을 재사용하는 스케줄러와 메모리 슬롯을 재사용하는 캐시 관리가 함께 동일 지연 조건의 처리량을 만든다는 점을 분석한다.

[[SGLang - Efficient Execution of Structured Language Model Programs]]의 [[접두사 KV 캐싱]]은 여러 호출에서 토큰 단위로 동일한 접두사의 KV 상태를 재사용해 중복 프리필을 줄인다. [[Fast Inference from Transformers via Speculative Decoding]]의 [[추측 디코딩]]은 작은 모델의 후보와 타깃 모델의 병렬 검증으로 타깃 분포를 유지하면서 직렬 디코드 횟수를 줄인다. [[재사용과 추측은 LLM 추론 작업량을 어떻게 바꾸는가]]는 캐시 보존과 드래프트·검증처럼 새로 생기는 비용까지 함께 센다.
...
[[Expectation vs. Experience: Evaluating the Usability of Code Generation Tools Powered by Large Language Models]], [[The Impact of AI on Developer Productivity: Evidence from GitHub Copilot]], [[Measuring the Impact of Early-2025 AI on Experienced Open-Source Developer Productivity]]은 서로 다른 과제·도구·개발자·완료 기준에서 AI 코딩 지원을 측정한다. [[자동화는 계산 노동을 없애는가 책임을 옮기는가]]는 이 결과를 하나의 생산성 배수로 합치지 않고, [[검증 노동]]과 통합 책임이 어디에 남는지로 다시 읽는다.
...
[[Brewer's Conjecture and the Feasibility of Consistent, Available, Partition-Tolerant Web Services]]는 분할이 있는 비동기 네트워크에서 원자적 일관성과 응답 보장을 함께 유지할 수 없다는 이론적 경계를 제시한다. [[In Search of an Understandable Consensus Algorithm]]은 리더 선출·로그 복제·안전성을 분리해 crash fault 아래의 복제 상태 기계를 설명하고, [[Spanner - Google's Globally-Distributed Database]]는 Paxos 복제, 다중 그룹 트랜잭션, TrueTime의 시간 불확실성을 결합한다. [[분산 서비스는 빠른 응답과 같은 상태를 어떻게 함께 보장하는가]]는 이 세 층을 사용자 기한·복구·종단 지연의 측정 경계로 종합한다.
...
[[데이터 표현과 인코딩]]은 [[인코딩]], [[이진법]], [[비트와 바이트]], [[ASCII]], [[비트 연산]]을 통해 디지털 데이터가 어떻게 표현되고 해석되는지 설명한다. [[C Integer and Shift Semantics]]와 [[CWE-190 Integer Overflow or Wraparound]]는 시프트·정수 폭·파생된 할당 크기의 조건을 보강한다. 이 주제는 [[C 문자열]], [[정수 오버플로]], [[메모리 안전성]]을 통해 낮은 수준의 표현 규칙이 시스템 안정성과 보안으로 이어짐을 보여준다. [[입력 검증]]과 [[비트 패턴과 해석 규칙]]은 이 관점을 인코딩, 타입, 경계 조건, 텍스트 처리 오류와 실제 소비 표현의 일치까지 확장한다.

### 인코딩과 유니코드 심화

[[The Unicode Standard 17.0.0]], [[RFC 3629 UTF-8]], [[Unicode Normalization Forms]], [[Unicode Text Segmentation]], [[WHATWG Encoding Standard]], [[Unicode Security Mechanisms]], [[CWE-20 Improper Input Validation]]는 텍스트 인코딩과 입력 계약의 세부 층위를 보강하는 참고 자료다. 이 축은 [[유니코드]], [[코드 포인트]], [[Unicode scalar value]], [[코드 유닛]], [[UTF-8]], [[UTF-16]], [[바이트 순서 표식]], [[유니코드 정규화]], [[그래핌 클러스터]], [[인코딩 오류]], [[동형이의 문자]], [[입력 검증]]을 통해 "문자"가 단일 단위가 아니라는 점과, 어느 표현을 검증·저장·비교·인가하는지가 보안 경계가 된다는 점을 정리한다. 핵심 분석은 [[인코딩 심화]]와 [[입력을 해석하는 경계는 왜 보안 경계인가]]에 모았다.
...
[[수학 표에서 스프레드시트로]]는 미리 계산된 수치 항목을 배포한 인쇄 수학 표와 셀 수식을 재계산하는 스프레드시트를 비교하고, VisiCalc의 고정 순회와 Excel의 의존성 기반 재계산을 구분해 표의 사용자가 조회자에서 계산 모형의 작성자·검토자로 바뀐 과정을 분석한다.
