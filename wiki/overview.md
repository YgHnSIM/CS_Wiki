...
역사·인과 렌즈는 같은 222개 공개 문서를 연도 목록이 아니라 **한계 → 대응 → 새 능력 → 다음 제약**의 변화로 읽게 한다. 현재 원전 대조로 연도가 기록된 문서는 142개이고, 그중 사건 시점을 가진 문서 38개와 출판 시점을 가진 문서 104개를 1800년 이전부터 2015년 이후까지 여덟 시대에 배치한다. 연도를 확인하지 못한 151개는 제목이나 작성일로 추정하지 않고 별도 `연도 미상` 시대에 보존한다. 편집자가 직접 기록한 `responds_to`, `enables`, `precedes`, `constrains`만 합성한 검토 전환은 17개이며 완전 전환 9개와 부분 전환 8개를 구분해 보여 준다.
...
근거 계보 렌즈는 196개 공개 지식 문서, 97개 정규 소스·참고 자료와 851개 문서–근거 연결을 **원자료·재현 정보 → 정규 소스·참고 자료 → 지식 문서·검토 관계**의 3레일로 펼친다. 프론트매터 `sources`는 문서 전체의 등록 근거 묶음으로, 설명과 직접 근거가 붙은 편집 관계 44개는 별도의 검토 관계로 구분한다. 근거 수를 신뢰도·진실도 점수로 만들지 않고, 같은 자료를 쓰는 문서도 공통 자료를 등록했다는 사실만 보여 준다. 근거 문서의 로컬 원본 6개, 보존 스냅샷 7개, 외부 링크 의존 84개는 품질 순위가 아니라 재현 조건으로 표시한다.
...
처음 읽는 사용자는 19개의 학습 경로에서 컴퓨팅의 기원, 계산 가능성, 알고리즘 복잡도와 실용성, 저장 프로그램 컴퓨터, 초기 소프트웨어, 구조적 프로그래밍, Fortran과 컴파일러, Unix와 C, 재사용과 API, 논리 회로, 유니코드, 컴퓨팅 능력 발달, 범용 기계와 프로그래밍 가능성, 더 빠른 프로세서의 조건, 부동소수점 성능과 정확성, 병렬 확장성의 두 질문, 전력 장벽과 에너지 효율, 서비스 성능과 꼬리 지연, 결함 허용과 가용성을 순서대로 따라갈 수 있다. [[컴퓨팅 능력 독서 지도]]는 이 경로들을 빠른 진입, 핵심 분석 시리즈와 일곱 층위별 원전 심화로 다시 조합해 중복 없이 읽게 한다. 개별 문서의 근거 추적 영역은 문서 단위 근거와 자료 페이지가 기록한 원자료 및 보조·접근 자료, 원본 보존 상태와 원문 URL을 한 흐름으로 연결한다.
