...
Initial Orders 1은 약 31개의 명령으로 [[로더]]와 [[어셈블러]]의 핵심 기능을 수행했다. `A100S`처럼 사람이 읽을 수 있는 명령 표기는 opcode, 주소, 길이 비트로 조립되어 메모리에 저장되었고, 저장 위치는 `m[25]`의 저장 명령을 고치는 [[자기 수정 코드]]로 갱신되었다.
