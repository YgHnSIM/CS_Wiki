...
[[CWE-20 Improper Input Validation]]은 제품이 입력이나 데이터를 받았을 때, 그 데이터를 안전하고 올바르게 처리하는 데 필요한 속성을 검증하지 않거나 잘못 검증하는 약점을 정리한 MITRE CWE 항목이다. 이 항목은 단일 언어의 버그가 아니라 입력이 내부 표현·연산·권한 결정으로 넘어가는 경계의 계약 문제를 다룬다.
