...
- Church는 효과적으로 계산 가능한 함수를 람다 정의 가능성 또는 재귀 함수 개념으로 형식화했다.
- 계산 가능성은 단순히 "사람이 언젠가 풀 수 있을 것 같다"는 직관이 아니라, 특정 형식 체계 안에서 다룰 수 있는 성질이 된다.
- 일부 수론 문제는 명확하게 서술되어 있어도 일반적인 계산 절차로 해결할 수 없다.
- Turing의 기계 모델과 Church의 함수 모델은 서로 다른 길로 계산 가능성의 같은 경계를 가리킨다.
