...
이 문서는 전력당 성능(PPW)을 LINPACK의 달성 최고 성능 `Rmax`를 그 실행 구간의 평균 시스템 전력 `P̄(Rmax)`로 나눈 GFLOPS/W로 정의한다. 성능과 전력은 동일한 시스템 규모와 동일한 문제 크기에서 측정해야 하며, 순간 전력이나 장비 정격 전력 대신 충분히 긴 실행 동안의 평균 AC RMS 전력을 사용한다.
