...
논문의 표면 주제는 계산 가능한 실수이지만, 진정한 중요성은 계산 절차 자체를 수학적 대상으로 만든 데 있다. Turing은 사람이 종이에 기호를 쓰고 지우며 일정한 규칙을 따르는 계산 과정을 테이프, 상태, 기호, 전이 규칙으로 모델링했다. 이 모델은 이후 [[계산 가능성]] 이론의 중심 언어가 되었다.
