...
핵심 구분은 재사용이 한 산출물을 다른 시스템에서 다시 쓰는 일인 반면, 이식성은 한 시스템을 다른 실행 환경으로 옮기는 일이라는 점이다. 이 구분은 이 위키의 [[Unix와 C]] 흐름과도 맞물린다. C와 Unix가 이식성을 높였다고 해서 그 자체가 곧 재사용은 아니며, 재사용은 산출물의 식별, 선택, 적용, 조직적 실천을 별도로 요구한다.
