...
캐시 공간과 실행 중 요청은 같은 메모리 풀을 공유한다. 메모리가 부족하면 사용 중이 아닌 잎 가운데 최근에 가장 적게 사용된 잎부터 제거하고, 대기 요청이 많을 때는 더 긴 공통 접두사를 가진 요청을 먼저 실행해 적중률을 높인다. 이 정책은 처리량을 높일 수 있지만 온라인 부하에서는 공정성과 지연을 해칠 수 있어, 논문도 기아 방지를 후속 과제로 남긴다.
