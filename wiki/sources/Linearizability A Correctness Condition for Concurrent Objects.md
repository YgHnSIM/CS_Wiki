...
이 조건은 구현이 내부적으로 완전히 직렬 실행되어야 한다고 요구하지 않는다. 여러 연산은 실제로 겹쳐 진행할 수 있지만, 관찰 가능한 연산 이력은 객체의 순차 명세와 모순되지 않는 하나의 순서로 설명돼야 한다. 그래서 기존의 순차 자료형 명세와 전제·사후 조건을 동시 객체의 추론에도 활용할 수 있다.
